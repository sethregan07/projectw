...
ssh root@102.215.228.243 "cd ${APP_DIR} && export VPS_IP=102.215.228.243 && ./deploy-pm2-nginx.sh" || {
    echo "❌ VPS deployment failed"
    echo "Attempting manual restart..."
    ssh root@102.215.228.243 "cd ${APP_DIR} && npm run build && pm2 restart nextjs-app && systemctl restart nginx"
    exit 1
}
...
echo "🌐 App URL: http://102.215.228.243/"
echo "👨‍💻 Check status: ssh root@102.215.228.243 'pm2 list'"
